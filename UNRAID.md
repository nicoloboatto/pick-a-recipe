...
`build-and-push.sh`, and `scripts/portainer-deploy.sh` back the upstream
project's own Docker Hub account and Portainer server (`.github/workflows/deploy.yml`)
— unrelated to, and untouched by, the GHCR/Unraid path this file describes.
Leave them alone; this fork's workflow (`docker-build.yml`) runs independently
alongside them.
